...
public:
  static std::shared_ptr<UIMarginBox> Create(std::weak_ptr<UIWindow> parent_window, int top, int right, int bottom, int left);
...
private:
  UIMarginBox(std::weak_ptr<UIWindow> parent_window, int top, int right, int bottom, int left);
